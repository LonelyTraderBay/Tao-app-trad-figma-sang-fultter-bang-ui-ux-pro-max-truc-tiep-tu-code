...
void main(List<String> args) {
  ...
  if (checkOnly) {
    if (!auditCsv.existsSync()) {
      ...
      stderr.writeln('Run `dart run tool/card_tile_audit.dart` from flutter_app/.');
      exitCode = 1;
      return;
    }
...
_AuditRow _auditFile(String relative, String source) {
  ...
  for (final block in blocks) {
    ...
    final allowStart = block.contains('card-tile: allow-start') ||
    ...
    final centered = params.contains('contentAlign: VitCardContentAlign.center');
  ...
  final status = tierAViolations > 0
      ...
      : (paddingTokenDebt > 0 || skeletonDrift > 0
            ? 'warn'
            : (fixedHeightCount > 0 &&
                  notes.contains('fixed_height_baseline')
              ? 'warn'
              : 'pass'));
  ...
  for (final row in canonical) {
    buffer.writeln(
      [
        ...
        'A',
        row.file,
        row.file.contains('compact_product') ? 'VitCompactProductCard' : 'VitMarketTickerCard',
        'canonical',
        'yes',
...
String _renderComplianceReport(List<_AuditRow> rows) {
  ...
  final buffer = StringBuffer()
    ...
    ..writeln()
    ..writeln('Generated by `dart run tool/card_tile_audit.dart` from `flutter_app/`.')
    ..writeln()
    ..writeln('## Summary')
    ..writeln()
  ...
  buffer.writeln('Showing first ${warnings.length} warn files (see CSV for full list).');
  buffer.writeln();
