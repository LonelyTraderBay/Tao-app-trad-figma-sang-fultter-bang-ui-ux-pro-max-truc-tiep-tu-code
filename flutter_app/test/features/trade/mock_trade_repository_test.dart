// Smoke test for MockTradeRepository (trade feature's own repository):
// exercises every SpotTradeRepository / TradeFuturesMarginRepository
// getter/action against the mock and asserts each call succeeds (doesn't
// throw) and returns a plausible result.
//
// `MockTradeRepository` delegates every call to trade_terminal's
// `MockTradeTerminalRepository` (see
// test/features/trade_terminal/mock_trade_terminal_repository_test.dart for
// the underlying fixture coverage), but it is a real distinct class wired
// via `tradeRepositoryProvider` in
// lib/app/providers/trade_controller_providers.dart, so it gets its own
// smoke coverage confirming the delegation wiring itself works end to end.
