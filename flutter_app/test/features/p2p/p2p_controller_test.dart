...
void main() {
  test('P2P create ad controller validates publish preview and limits', () {
  ...
  test('P2P risk controller exposes material review factors', () {
  ...
  test('P2P payment edit controllers own ownership and cooling state', () {
    final repository = const MockP2PRepository();
    final ownership = P2PPaymentMethodOwnershipController(
      state: P2PPaymentMethodOwnershipViewState(
        snapshot: repository.getPaymentMethodOwnership('sample'),
      ),
    );
    ...
    final cooling = P2PPaymentMethodCoolingPeriodController(
      state: P2PPaymentMethodCoolingPeriodViewState(
        snapshot: repository.getPaymentMethodCoolingPeriod(),
      ),
    );
  ...
  test('P2P dispute evidence controller owns upload and submit state', () {
  ...
  test('P2P order and express confirm controllers own confirmation intent', () {
    final repository = const MockP2PRepository();
    final orderController = P2POrderController(
      state: P2POrderViewState(snapshot: repository.getOrder('p2p001')),
    );
    ...
    final expressController = P2PExpressConfirmController(
      state: P2PExpressConfirmViewState(
        snapshot: repository.getExpressConfirm(adId: 'ad001'),
      ),
    );
