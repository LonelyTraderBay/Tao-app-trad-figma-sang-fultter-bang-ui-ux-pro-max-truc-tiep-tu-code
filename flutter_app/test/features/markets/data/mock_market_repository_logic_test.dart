...
void main() {
  const repo = MockMarketRepository(loadDelay: Duration.zero);

  group('MockMarketRepository smoke test', () {
    group('screener sort & filter logic', () {
      test('default query sorts by marketCap desc', () async {
        final snapshot = await repo.getMarketScreener();
      ...
      test('searchQuery filters by baseAsset/symbol substring', () async {
        final result = await repo.getMarketScreener(
      ...
      test('categories filter narrows to matching category', () async {
        final result = await repo.getMarketScreener(
      ...
      test('minPrice filters out pairs below the threshold', () async {
        final result = await repo.getMarketScreener(
      ...
      test('sortDirection reverses the change24h ordering', () async {
        final asc = await repo.getMarketScreener(
        ...
        final desc = await repo.getMarketScreener(
          query: const MarketScreenerQuery(
            sortBy: MarketScreenerSort.change24h,
    ...
    group('calendar sort & filter logic', () {
      test(
        'default query returns all events sorted by date ascending',
        () async {
      ...
      test('type filter narrows and preserves date ordering', () async {
        final result = await repo.getMarketCalendar(
      ...
      test('impact filter narrows to high-impact events', () async {
        final result = await repo.getMarketCalendar(
    ...
    group('derivatives sort logic', () {
      test('every MarketDerivativesSort criterion orders correctly', () async {
        final byOi = await repo.getMarketDerivatives();
        expect(byOi, isA<MarketDerivativesSnapshot>());
        expect(byOi.pairs[0].id, 'btc-perp');
        expect(byOi.pairs[1].id, 'eth-perp');

        final byVolume = await repo.getMarketDerivatives(
    ...
    group('portfolio sort logic', () {
      test('every MarketPortfolioSort criterion orders correctly', () async {
        final byValue = await repo.getPortfolioTracker();
    ...
    group('market news filter logic', () {
      test('category=all / breaking / specific category', () async {
        final all = await repo.getMarketNews();
        ...
        final bitcoin = await repo.getMarketNews(category: 'bitcoin');
        expect(bitcoin.news.map((n) => n.id), ['n1', 'n9']);
      });

      test('sentiment filter combines with category filter', () async {
        final bearish = await repo.getMarketNews(
          sentiment: MarketNewsSentiment.bearish,
        );
        expect(bearish.news.map((n) => n.id), ['n5', 'n9']);

        final altcoinBullish = await repo.getMarketNews(
          category: 'altcoin',
    ...
    group('advanced charts category filter logic', () {
      test(
        'indicatorCategory and drawingCategory narrow independently',
        () async {
          final all = await repo.getAdvancedCharts();
          expect(all, isA<MarketAdvancedChartsSnapshot>());
          expect(all.indicators, hasLength(10));
          expect(all.drawingTools, hasLength(12));

          final trend = await repo.getAdvancedCharts(
            indicatorCategory: 'trend',
          );
          expect(trend.indicators.map((i) => i.id), ['sma', 'ema', 'ichimoku']);

          final line = await repo.getAdvancedCharts(drawingCategory: 'line');
    ...
    group('token unlocks sort & filter logic', () {
      test('every MarketUnlockSort criterion orders correctly', () async {
        final byNearest = await repo.getTokenUnlocks();
        ...
        final byValue = await repo.getTokenUnlocks(
          sortBy: MarketUnlockSort.value,
        );
        expect(byValue.unlocks[0].id, 'u5');
        expect(byValue.unlocks[1].id, 'u1');

        final byImpact = await repo.getTokenUnlocks(
          sortBy: MarketUnlockSort.impact,
        );
        expect(byImpact.unlocks[0].id, 'u5');
        expect(byImpact.unlocks[1].id, 'u6');
      });

      test('impactFilter narrows and re-sorts by the active sortBy', () async {
        ...
        final mediumImpact = await repo.getTokenUnlocks(
          impactFilter: MarketUnlockImpact.medium,
        );
        expect(mediumImpact.unlocks.map((u) => u.id), ['u2', 'u4', 'u6']);

        final lowImpact = await repo.getTokenUnlocks(
    ...
    group('social signals filter logic', () {
      test('statusFilter and categoryFilter narrow independently', () async {
        final all = await repo.getSocialSignals();
        ...
        final active = await repo.getSocialSignals(
          statusFilter: TradingSignalStatus.active,
        );
        expect(active.signals, hasLength(5));

        final swing = await repo.getSocialSignals(
      ...
      test(
        'combined statusFilter + categoryFilter applies AND logic',
        () async {
          final activeSwing = await repo.getSocialSignals(
            statusFilter: TradingSignalStatus.active,
            categoryFilter: TradingSignalCategory.swing,
          );
    ...
    group('market correlations sort logic', () {
      test('sortOrder controls high/low ranking per timeframe', () async {
        final high7d = await repo.getMarketCorrelations();
