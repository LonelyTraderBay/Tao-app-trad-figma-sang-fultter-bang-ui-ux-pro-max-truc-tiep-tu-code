...
void main() {
  ...
  group('MockMarketRepository smoke test', () {
    ...
    group('pair & token detail getters', () {
      test('getPairDetail returns the matching pair and falls back', () async {
        final known = await repo.getPairDetail('ethusdt');
        expect(known, isA<MarketPairDetailSnapshot>());
        expect(known.pair.id, 'ethusdt');
        expect(known.depth.bids, hasLength(25));

        final fallback = await repo.getPairDetail('does-not-exist');
        expect(fallback.pair.id, 'btcusdt');
      });

      test(
        'getTokenInfo returns fundamentals and falls back to BTC data',
        () async {
          final btc = await repo.getTokenInfo('btcusdt');
          ...
          final eth = await repo.getTokenInfo('ethusdt');
    ...
    group('market depth level normalization', () {
      test('accepts allowed levels and falls back to 25 otherwise', () async {
        final defaultDepth = await repo.getMarketDepth();
        ...
        final level15 = await repo.getMarketDepth(levels: 15);
        expect(level15.depth.bids, hasLength(15));
        expect(level15.depth.asks, hasLength(15));

        final level50 = await repo.getMarketDepth(
          pairId: 'ethusdt',
          levels: 50,
        );
        ...
        final invalidLevel = await repo.getMarketDepth(levels: 999);
