...
void main() {
  ...
  group('MockTradeBotsRepository lifecycle & risk smoke test', () {
    test('getTradingBots pins highRiskContractId and active bot count', () {
    ...
    test('getBotTermsOfService / getBotRiskDisclosure pin fixture copy', () {
      final terms = repo.getBotTermsOfService();
      expect(terms.title, 'Trading Bots Terms of Service');
      expect(terms.sections, isNotEmpty);

      final risk = repo.getBotRiskDisclosure();
      expect(risk.highRiskTitle, 'HIGH RISK WARNING');
      expect(risk.categories, isNotEmpty);
    });

    test('getBotSuitabilityAssessment returns populated questions', () {
      final assessment = repo.getBotSuitabilityAssessment();
      expect(assessment.questions, isNotEmpty);
      expect(assessment.endpoint, isNotEmpty);
    });

    test('getBotRiskDashboard pins the risk score and running bot count', () {
      final dashboard = repo.getBotRiskDashboard();
      expect(dashboard.riskScore, 68);
      expect(dashboard.runningBots, 3);
      expect(dashboard.emergencyPath, '/trade/bots/emergency-stop');
    });

    test('getBotEmergencyStop / getBotSecuritySettings pin fixture data', () {
      final emergency = repo.getBotEmergencyStop();
      expect(emergency.bots, hasLength(3));
      expect(emergency.reasons, isNotEmpty);

      final security = repo.getBotSecuritySettings();
      expect(security.twoFaEnabled, isTrue);
      expect(security.apiKeys, isNotEmpty);
    });

    test('getBotHistory / getBotPerformanceAnalytics pin fixture counts', () {
      final history = repo.getBotHistory();
      expect(history.trades, hasLength(7));

      final performance = repo.getBotPerformanceAnalytics();
