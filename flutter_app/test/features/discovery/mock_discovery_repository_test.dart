...
void main() {
  ...
  group('MockDiscoveryRepository smoke test', () {
    ...
    test('getUnifiedSearch does not throw for an unmatched query and falls '
      ...
      late final UnifiedSearchSnapshot snapshot;

      expect(
        () => snapshot = repository.getUnifiedSearch(query: 'zzz-no-match'),
        returnsNormally,
      );
      expect(snapshot, isA<UnifiedSearchSnapshot>());
    ...
    test('getTopicHub with defaults returns the crypto topic snapshot', () {
    ...
    test('getTopicHub does not throw for an unrecognized topic id and falls '
        'back to the crypto topic', () {
      late final TopicHubSnapshot snapshot;

      expect(
        () => snapshot = repository.getTopicHub(topicId: 'does-not-exist'),
        returnsNormally,
      );
      expect(snapshot, isA<TopicHubSnapshot>());
      expect(snapshot.selectedTopic.id, 'crypto');
