...
void main() {
  group('state management guardrails', () {
    test(
      'Provider.family with a non-scalar key uses autoDispose',
      () {
        ...
        for (final file in Directory('lib/app/providers').listSync().whereType<File>()) {
    ...
    test(
      'presentation pages do not seed a mutable local list from ref.read '
      '(dual-source state)',
      () {
        ...
        final violations = <String>[];
        for (final file
            in Directory('lib/features').listSync(recursive: true).whereType<File>()) {
          final path = file.path.replaceAll('\\', '/');
