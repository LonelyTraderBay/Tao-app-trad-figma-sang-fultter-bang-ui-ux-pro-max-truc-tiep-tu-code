...
class _LaunchpadBridgeComparePageState
  ...
  List<LaunchpadBridgeRouteOptionDraft> _sortedRoutes(
    ...
    switch (_sortMode) {
      ...
      case 'recommended':
      default:
        sorted.sort((a, b) => _recommendedRank(a).compareTo(_recommendedRank(b)));
        break;
    }
    return sorted;
  }
    ...
    return VitCard(
      ...
      child: Column(
        children: [
          ...
          if (expanded) _ExpandedRouteDetails(route: route, comparison: comparison),
        ],
      ),
    );
  }
...
class _RankedProviderBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        _ProviderBadge(label: route.providerIcon, accent: route.accent, size: 40),
        if (rank == 1)
          Positioned(
            top: -6,
            right: -6,
...
class _RouteMetric extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: AppTextStyles.micro.copyWith(color: AppColors.text3)),
        const SizedBox(height: AppSpacing.x1),
        Text(
          value,
          maxLines: 1,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.x3),
          _DetailsRow(label: 'Gas cost', value: '\$${route.gasCost.toStringAsFixed(2)}'),
          _DetailsRow(label: 'Bridge fee', value: '\$${route.bridgeFee.toStringAsFixed(2)}'),
          _DetailsRow(label: 'Price impact', value: '${_trimDouble(route.priceImpact)}%'),
          _DetailsRow(label: 'Slippage tolerance', value: '${_trimDouble(route.slippage)}%'),
    ...
    return VitStickyFooter(
      ...
      child: Column(
        children: [
          Row(
            children: [
              _ProviderBadge(label: route.providerIcon, accent: route.accent, size: 34),
              const SizedBox(width: AppSpacing.x3),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _DetailsRow(label: 'Security', value: '${route.securityScore}/100'),
                const SizedBox(height: AppSpacing.x4),
                Container(
