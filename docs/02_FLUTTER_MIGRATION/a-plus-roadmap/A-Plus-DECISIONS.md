...
| **DEC-backend** | Mốc thời gian backend thật về | Ngày cụ thể — dùng để khoá deadline Giai đoạn 2 (mọi task "Viết lại" GĐ2 phải xong **trước** mốc này) | _(chưa — chỉ user biết lịch backend)_ | `pending` | | Không chặn task đơn lẻ; chặn việc lập lịch GĐ2 |
...
| **DEC-ios** | iOS có trong scope release không | **Có** (thêm job `flutter build ipa` trên macos-latest — cần runner macOS) · **Không/để sau** (chỉ Android) | _(chưa — cần user xác nhận có Apple developer account/nhu cầu phát hành iOS)_ | `pending` | | CI-D6 (chỉ phần job iOS; flavors/release Android làm được trước) |
