...
| 2 | Create Flutter design tokens | `Flutter-Design-Tokens.md`, `src/styles/theme.css`, `src/app/hooks/useThemeColors.ts` | Implement `AppColors`, `AppGradients`, `AppSpacing`, `AppRadii`, `AppTextStyles`, `DeviceMetrics` | Tokens match documented values and are used by sample app shell | Stage 3 |
...
| 6 | Port `SC-007 HomePage` as shell proof | `Flutter-Port-Master-Plan.md` row `SC-007`, Home screenshots, Home React source, Home screen references under `docs/04_SCREEN_REFERENCES/home/` | Implement only `SC-007 HomePage`, mock data first, route `/home`, no unrelated screens | Home viewport and full-page Flutter captures are visually checked against both references | Stage 7 |
| 7 | Start screen-by-screen migration loop | `Flutter-Port-Master-Plan.md`, `Flutter-Visual-QA.md`, target screen React source | Pick the next `SC-xxx` by master plan phase/order, implement one screen, wire only needed navigation, add mock repository contract | One target screen has Flutter UI, route, BE draft, and visual QA checked | Stage 8 |
...
4. Check shared tokens/components/routing docs.
...
8. Capture Flutter viewport and full-page screenshots.
9. Compare against the React baseline.
10. Tick status only when Flutter UI, navigation, BE draft, and visual QA are actually complete.
