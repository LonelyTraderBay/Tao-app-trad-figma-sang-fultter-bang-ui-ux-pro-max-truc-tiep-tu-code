...
| UI source | React screenshots/source for structure and behavior; Home native standard for Flutter runtime color, size, chrome, and shared treatment |
...
`SC-007 HomePage` is the current approved Flutter native UX standard for the phone runtime. It controls later screen implementations for native safe areas, compact bottom navigation, scroll-driven chrome hiding, neutral dark surfaces, shared component sizing, and the Bybit-inspired dark orange Flutter brand palette. See `docs/02_FLUTTER_MIGRATION/Flutter-Native-Design-Standard.md` and `docs/04_SCREEN_REFERENCES/home/HomePage-Flutter-Native-Standard.md`.
...
3. Implement dark theme tokens from `Flutter-Native-Design-Standard.md` and `Flutter-Design-Tokens.md`.
...
- Do not copy legacy React blue palettes or screen-local sizing into native Flutter. Normalize to the Home native standard after visual-QA structure is matched.
- Do not add per-module active bottom-nav colors; the active bottom-nav state uses the Home brand token everywhere.
