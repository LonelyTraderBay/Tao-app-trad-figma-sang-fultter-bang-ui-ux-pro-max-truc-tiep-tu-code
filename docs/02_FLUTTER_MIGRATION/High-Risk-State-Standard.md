...
## The 8 pages the guardrail checks
...
- The test only checks this **fixed 8-file allowlist**. AGENTS.md's
  Financial Safety section also names security changes and address
  additions and P2P payment-method changes — those pages are not currently
  covered by this guardrail even though the policy text applies to them.
  Extending the target set (or replacing it with a directory scan) is
  unimplemented. Two concrete examples of the gap, at different stages:
  - `lib/features/profile/presentation/pages/security_page.dart:123` already
    renders `VitHighRiskStatePanel(state: VitHighRiskUiState.riskReview, ...)`,
    but wires `contractId: 'Điểm bảo mật: ${snapshot.score}/4'` — a formatted
    score string, not a `highRiskContractId` field — and renders unconditionally
    instead of behind `if (snapshot.highRiskContractId != null)`. It follows
    the panel convention but not the contract-id/nullable-gating convention
    from Rule 2.
  - `lib/features/wallet/presentation/pages/address_add_page.dart` has no
    `VitHighRiskStatePanel` and no ad-hoc warning banner at all — a full gap,
    not just a wiring mismatch.
